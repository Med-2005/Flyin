...
class SimulationRouter:
    def __init__(self, zones: Dict[str, 'Zone'], graph: Dict[str, List[str]], link_caps: Dict[Tuple[str, str], int], start_zone: str):
        self.zones = zones
        self.graph = graph
    ...
    def is_link_available(self, curr_zone: str, next_zone: str, start_turn: int, turns_needed: int) -> bool:
        ...
        for t in range(start_turn, start_turn + turns_needed):
            current_usage = self.link_reservations.get((curr_zone, next_zone), {}).get(t, 0)
            current_usage += self.link_reservations.get((next_zone, curr_zone), {}).get(t, 0)
    ...
    def reserve_step(self, curr_zone: str, next_zone: str, start_turn: int, arrival_turn: int, turns_needed: int):
        link = (curr_zone, next_zone)
        ...
        for t in range(start_turn, start_turn + turns_needed):
            self.link_reservations[link][t] = self.link_reservations[link].get(t, 0) + 1
        
        self.reservations[next_zone][arrival_turn] = self.reservations[next_zone].get(arrival_turn, 0) + 1

    def find_dynamic_step(self, start: str, end: str, start_turn: int, max_turns: int = 1000) -> Optional[Tuple[str, int]]:
        ...
        while pq:
            ...
            for neighbor in self.graph.get(curr_zone, []):
                ...
                if self.is_zone_available(neighbor, arrival_turn) and self.is_link_available(curr_zone, neighbor, curr_turn, turns_needed):
